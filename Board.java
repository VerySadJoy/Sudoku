...
{
   static FinalProject proj = new FinalProject();
   ...
   // Grid colours 
   private static final Color GRID_COLOR_A = new Color (75,0,130);
   private static final Color GRID_COLOR_B = new Color (147,112,219);
   
   // Preset colours for pieces
   private static final Color[] COLOURS = 
   
     {Color.YELLOW, Color.BLUE, Color.CYAN, Color.GREEN, 
       Color.PINK, Color.WHITE, Color.RED, Color.LIGHT_GRAY };
   
   // String used to indicate each colour
   private static final String[] COLOUR_NAMES = 
    {"yellow", "blue", "cyan", "green", "pink", "white", "red", "gray"};
   ...
   private void drawLine(Graphics g)
   {
      ...
      {
         ((Graphics2D) g).setStroke(new BasicStroke(8.0f * (float)scale));
         g.drawLine((int)Math.round((X_OFFSET+X_DIM/20.0+line[0][i]*X_DIM)*scale) - 3, 
                      (int)Math.round((Y_OFFSET+Y_DIM/20.0+line[1][i]*Y_DIM)*scale) - 3, 
                      (int)Math.round((X_OFFSET+X_DIM/20.0+line[2][i]*X_DIM)*scale) - 3, 
                      (int)Math.round((Y_OFFSET+Y_DIM/20.0+line[3][i]*Y_DIM)*scale) - 3);
      }
   }   
   /**
   ...
   public void drawLine(int row1, int col1, int row2, int col2)
   {
      ...
      this.line[3][numLines]=row2 + 1;
      this.numLines++;
      repaint();
   }
   public void hdrawLine(int row1, int col1, int row2, int col2)
   {
      this.line[0][numLines]=col1;
      this.line[1][numLines]=row1;
      this.line[2][numLines]=col2 + 1;
